...
class PromptValidator {
    ...
    checkPrompts(source, preparedActions, isPower = false, powerCost = 0) {
        ...
        const ourCardsInPlay = getZone(const_1.ZONE_TYPE_IN_PLAY).cards.filter(card => (creatureWillSurvive ? true : card.id !== source.id) && modifyByStaticAbilities(card, const_1.PROPERTY_CONTROLLER) === source.data.controller);
        const allCardsInPlay = getZone(const_1.ZONE_TYPE_IN_PLAY).cards.filter(card => creatureWillSurvive ? true : card.id !== source.id);
        const metaValues = {
            '$source': source,
        ...
        const allPromptsAreDoable = allPrompts.every(promptAction => {
            switch (promptAction.promptType) {
                ...
                case const_1.PROMPT_TYPE_MAGI_WITHOUT_CREATURES:
                    ...
                    const magi = [...getZone(const_1.ZONE_TYPE_ACTIVE_MAGI, source.data.controller).cards, ...getZone(const_1.ZONE_TYPE_ACTIVE_MAGI, opponent).cards];
                    return magi.some(magi => !allCardsInPlay.some(card => card.card.type === const_1.TYPE_CREATURE && modifyByStaticAbilities(card, const_1.PROPERTY_CONTROLLER) === magi.data.controller));
                ...
                case const_1.PROMPT_TYPE_ANY_CREATURE_EXCEPT_SOURCE: {
                    return getZone(const_1.ZONE_TYPE_IN_PLAY).cards.some(card => card.id !== source.id);
                }
                case const_1.PROMPT_TYPE_POWER_ON_MAGI: {
                    const magi = getZone(const_1.ZONE_TYPE_ACTIVE_MAGI, source.data.controller).cards;
                ...
                case const_1.PROMPT_TYPE_SINGLE_CREATURE_FILTERED: {
                    if ('restrictions' in promptAction.promptParams && promptAction.promptParams.restrictions) {
                        ...
                        return checkAnyCardForRestrictions(allCardsInPlay.filter(card => card.card.type === const_1.TYPE_CREATURE), restrictionsWithValues);
                    }
                        ...
                        switch (promptAction.promptParams.restriction) {
                            case const_1.RESTRICTION_OWN_CREATURE: {
                                return checkAnyCardForRestriction(allCardsInPlay.filter(card => card.card.type === const_1.TYPE_CREATURE), promptAction.promptParams.restriction, source.data.controller);
                            }
                            ...
                            default: {
                                ...
                                return checkAnyCardForRestriction(allCardsInPlay.filter(card => card.card.type === const_1.TYPE_CREATURE), promptAction.promptParams.restriction, restrictionValue);
                            }
                        }
                    }
                    return true;
                ...
                case const_1.PROMPT_TYPE_CHOOSE_N_CARDS_FROM_ZONE: {
                    ...
                    if (promptAction.promptParams.restrictions) {
                        ...
                        switch (promptAction.promptParams.restriction) {
                            case const_1.RESTRICTION_OWN_CREATURE: {
                                return checkAnyCardForRestriction(cardsInZone.filter(card => card.card.type === const_1.TYPE_CREATURE), promptAction.promptParams.restriction, source.data.controller);
                            }
                            case const_1.RESTRICTION_OPPONENT_CREATURE: {
                                return checkAnyCardForRestriction(cardsInZone.filter(card => card.card.type === const_1.TYPE_CREATURE), promptAction.promptParams.restriction, source.data.controller);
                            }
                            default: {
                                return checkAnyCardForRestriction(cardsInZone.filter(card => card.card.type === const_1.TYPE_CREATURE), promptAction.promptParams.restriction, promptAction.promptParams.restrictionValue);
                            }
                        }
                    }
                    return true;
