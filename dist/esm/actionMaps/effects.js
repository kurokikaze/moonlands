...
var applyPromptEnteredEffect = function (action) {
    var _this = this;
    if (!('player' in action)) {
        throw new Error('Prompt without player!');
    }
    var savedActions = this.state.actions;
    var promptParams = {};
    ...
    if (!skipPrompt) {
        this.state = __assign(__assign({}, this.state), { savedActions: savedActions, 
            // This will be the only action to fire after entering the prompt
            actions: [ /*this.convertPromptActionToEffect(action)*/], prompt: true, promptMessage: ('message' in action) ? action.message : '', promptPlayer: promptPlayer, promptType: action.promptType, promptVariable: action.variable, promptGeneratedBy: action.generatedBy, promptParams: promptParams });
    }
};
...
var applyDistributeCardsInZonesEffect = function (action, transform) {
...
var applyPromptEntered = function (action) {
    var promptPlayer = this.getMetaValue(action.player, action.generatedBy);
    this.state = __assign(__assign({}, this.state), { prompt: true, promptType: action.promptType, 
        // @ts-ignore
        promptParams: action.promptParams || this.state.promptParams, promptGeneratedBy: action.generatedBy, promptMessage: action.message, promptPlayer: promptPlayer, promptVariable: action.variable });
};
