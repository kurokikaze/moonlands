...
export type ActionTransformer<T> = (this: State, action: FilterFromUnion<EffectsUnited, { effectType: T }>, transform: (...args: AnyEffectType[]) => void, state: StateShape, nanoid: () => string) => void;

export type ActionHandlerMap = {
    [K in EffectsUnited['effectType']]: ActionTransformer<K>;
};

// Helper types for proto actions
...
export type ProtoEffectType = ProtoSelectEffect |
    ProtoEnergizeEffect |
