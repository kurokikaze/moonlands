...
export const applyAttackEffect: ActionTransformer<typeof EFFECT_TYPE_ATTACK> = function (action, transform) {
    ...
    const attackSequence: AnyEffectType[] = [
        {
            type: ACTION_EFFECT,
...
export const applyDamageStepEffect: ActionTransformer<typeof EFFECT_TYPE_DAMAGE_STEP> = function (action, transform) {
    ...
    const attackerDamageActions: (AttackerDealsDamageEffect | AttackerDamageDealtEffect | DefenderDealsDamageEffect | DefenderDamageDealt)[] = [{	// from source to target
        type: ACTION_EFFECT,
