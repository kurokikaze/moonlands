...
export class Unmaker {
    ...
    private saveActionType(t: number, _tag: string) {
    ...
    private readString(expectedTag: string): string {
        const tag = this.dataTags.pop()
        if (tag != expectedTag) {
            throw new Error(`Expected tag ${expectedTag} but found ${tag}`)
        }
                ...
                switch (action.effectType) {
                    ...
                    case EFFECT_TYPE_DIE_ROLLED: {
                        if (action.generatedBy) {
                            ...
                            this.saveNumber(currentMeta?.roll_result as number, 'EFFECT_TYPE_DIE_ROLLED/rollResult')
                    ...
                    case EFFECT_TYPE_START_TURN: {
                        ...
                        this.saveNumber(this.state.state.step as number, 'EFFECT_TYPE_START_TURN/step')
                        ...
                        this.saveNumber(this.state.turn as number, 'EFFECT_TYPE_START_TURN/turn')
                    ...
                    case EFFECT_TYPE_START_STEP: {
                        this.saveNumber(this.state.state.step as number, 'EFFECT_TYPE_START_STEP/step')
                    ...
                    case EFFECT_TYPE_REARRANGE_CARDS_OF_ZONE: {
                        ...
                        const cardsOrder: string[] | null = this.state.getMetaValue(action.cards, action.generatedBy)

                        if (!cardsOrder) return undefined
                    ...
                    case EFFECT_TYPE_FIND_STARTING_CARDS: {
                        const currentMeta = this.state.getSpellMetadata(action.generatedBy)
                        this.saveObject(currentMeta?.foundCards as unknown as string[] | undefined, 'EFFECT_TYPE_FIND_STARTING_CARDS/foundCards')
    ...
    public readAndApplyUnAction(state: State) {
        ...
        switch (unAction) {
            ...
            case UNMAKE_PROMPT_LEAVE: {
                const logCount = this.readNumber('logCount')
                const promptType = this.readString('promptType') as PromptTypeType
            ...
            case UNMAKE_PROMPT_ENTER: {
                const savedActions = this.readObject<any[]>('ACTION_ENTER_PROMPT/savedActions')
                const actions = this.readObject<any[]>('ACTION_ENTER_PROMPT/actions')
            ...
            case UNMAKE_EFFECT_TYPE_ADD_ENERGY_TO_CREATURE: {
                ...
                const creatures = this.readObject<{ id: string, energy: number }[]>('EFFECT_TYPE_ADD_ENERGY_TO_CREATURE/creatures')
            ...
            case UNMAKE_EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES: {
                const cardsWithPositions = this.readObject<{ card: CardInGame, position: number }[]>('EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES/cardsWithPositions')
                const sourceZoneType = this.readString('EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES/sourceZoneType') as ZoneType
                const zoneOwner = this.readNumber('EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES/zoneOwner')
                const destZoneType = this.readString('EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES/destZoneType') as ZoneType
                const bottom = this.readNumber('EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES/bottom') === 1
                const metaDataEntries = this.readObject<{ spellId: string, field: string, previousValue: any }[]>('EFFECT_TYPE_MOVE_CARDS_BETWEEN_ZONES/metaDataEntries')

                if (!cardsWithPositions || !metaDataEntries) break;
            ...
            case UNMAKE_EFFECT_TYPE_MOVE_CARD_BETWEEN_ZONES: {
                const zoneChangingCard = this.readObject<CardInGame>('EFFECT_TYPE_MOVE_CARD_BETWEEN_ZONES/zoneChangingCard')
            ...
            case UNMAKE_EFFECT_TYPE_DISCARD_ENERGY_FROM_CREATURE: {
                ...
                const creatures = this.readObject<{ id: string, energy: number, energyLostThisTurn: number }[]>('EFFECT_TYPE_DISCARD_ENERGY_FROM_CREATURE/creatures')
            ...
            case UNMAKE_EFFECT_TYPE_DISCARD_ENERGY_FROM_MAGI: {
                ...
                const magi = this.readObject<{ id: string, owner: number, energy: number, energyLost: number }[]>('EFFECT_TYPE_DISCARD_ENERGY_FROM_MAGI/magi')
                if (!magi) { state.state.log.length -= logCount; break; }
            ...
            case UNMAKE_EFFECT_TYPE_START_TURN: {
                ...
                const continuousEffect = this.readObject<any[]>('EFFECT_TYPE_START_TURN/continuousEffects')
                const cardFlags = this.readObject<Record<string, { wasAttacked: boolean, hasAttacked: boolean, attacked: number, actionsUsed: string[], defeatedCreature: boolean, energyLostThisTurn: number }>>('EFFECT_TYPE_START_TURN/cardFlags')
            ...
            case UNMAKE_EFFECT_TYPE_REARRANGE_CARDS_OF_ZONE: {
                const zone = this.readString('EFFECT_TYPE_REARRANGE_CARDS_OF_ZONE/zone') as ZoneType
                const owner = this.readNumber('EFFECT_TYPE_REARRANGE_CARDS_OF_ZONE/zoneOwner')
                const previousOrder = this.readObject<string[]>('EFFECT_TYPE_REARRANGE_CARDS_OF_ZONE/previousOrder')
            ...
            case UNMAKE_EFFECT_TYPE_CREATURE_DEFEATS_CREATURE: {
                ...
                const sourceCard = this.readObject<CardInGame>('EFFECT_TYPE_CREATURE_DEFEATS_CREATURE/source')
            ...
            case UNMAKE_EFFECT_TYPE_ADD_ENERGY_TO_MAGI: {
                ...
                const magiArray = this.readObject<{ id: string, owner: number, energy: number }[]>('EFFECT_TYPE_ADD_ENERGY_TO_MAGI/magiArray')
                if (!magiArray) { state.state.log.length -= logCount; break; }
            ...
            case UNMAKE_EFFECT_TYPE_START_OF_TURN: {
                ...
                const cardFlags = this.readObject<Record<string, { actionsUsed: string[], wasAttacked: boolean, hasAttacked: boolean, attacked: number, defeatedCreature: boolean, energyLostThisTurn: number }>>('EFFECT_TYPE_START_TURN/cardFlags')
            ...
            case UNMAKE_EFFECT_TYPE_FIND_STARTING_CARDS: {
                ...
                const foundCards = this.readObject<string[] | undefined>('EFFECT_TYPE_FIND_STARTING_CARDS/foundCards')
                state.state.log.length -= logCount
            ...
            case UNMAKE_EFFECT_TYPE_RESHUFFLE_DISCARD: {
                ...
                const discardCards = this.readObject<CardInGame[]>('EFFECT_TYPE_RESHUFFLE_DISCARD/discardCards')
                const deckCards = this.readObject<CardInGame[]>('EFFECT_TYPE_RESHUFFLE_DISCARD/deckCards')
            ...
            case UNMAKE_EFFECT_TYPE_REARRANGE_ENERGY_ON_CREATURES: {
                const creatures = this.readObject<{ id: string, energy: number }[]>('EFFECT_TYPE_REARRANGE_ENERGY_ON_CREATURES/creatures')
                const inPlay = state.getZone(ZONE_TYPE_IN_PLAY)
            ...
            case UNMAKE_EFFECT_TYPE_DISTRIBUTE_ENERGY_ON_CREATURES: {
                const creatures = this.readObject<{ id: string, energy: number }[]>('EFFECT_TYPE_DISTRIBUTE_ENERGY_ON_CREATURES/creatures')
                const inPlay = state.getZone(ZONE_TYPE_IN_PLAY)
                creatures.forEach(({ id, energy }) => {
                    const creature = inPlay.byId(id)
                    if (creature) {
            ...
            case UNMAKE_EFFECT_TYPE_FORBID_ATTACK_TO_CREATURE: {
                const creatures = this.readObject<{ id: string, attacked: number }[]>('EFFECT_TYPE_FORBID_ATTACK_TO_CREATURE/creatures')
                const inPlay = state.getZone(ZONE_TYPE_IN_PLAY)
