...
describe('Abaquist', () => {
  it('Posess', () => {
    ...
    expect(seenActions[2].promptParams.restrictions[0].type).toEqual(RESTRICTION_ENERGY_LESS_THAN);
    expect(seenActions[2].promptParams.restrictions[0].value).toEqual(6);
...
describe('Blu', () => {
  it('Artifice', () => {
    ...
    const blu = new CardInGame(byName('Blu'), ACTIVE_PLAYER).addEnergy(7);
    const pharan = new CardInGame(byName('Pharan'), ACTIVE_PLAYER).addEnergy(1);
    const belt = new CardInGame(byName('Orothean Belt'), ACTIVE_PLAYER);
    const seaBarl = new CardInGame(byName('Sea Barl'), ACTIVE_PLAYER);

    const zones = createZones(ACTIVE_PLAYER, NON_ACTIVE_PLAYER, [pharan], [blu]);
    ...
    gameState.setPlayers(ACTIVE_PLAYER, NON_ACTIVE_PLAYER);
    gameState.getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER).add([belt]);
    gameState.getZone(ZONE_TYPE_DECK, ACTIVE_PLAYER).add([seaBarl]);
    ...
    expect(gameState.state.prompt).toEqual(true, 'Game is in prompt state')
    expect(gameState.state.promptType).toEqual(PROMPT_TYPE_MAY_ABILITY, 'Prompt type is "may ability"')
    expect(gameState.state.promptGeneratedBy).toEqual(blu.id, 'Prompt is generated by Blu')
    ...
    expect(gameState.getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER).length).toEqual(1, 'Player has a new card in hand')
    expect(gameState.getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER).card.card.name).toEqual('Sea Barl', 'It is Sea Barl')
    expect(blu.data.energy).toEqual(6, 'Blu has 6 energy now')
  })

  it('Artifice (no energy on Blu)', () => {
    const ACTIVE_PLAYER = 422;
    const NON_ACTIVE_PLAYER = 1310;

    const blu = new CardInGame(byName('Blu'), ACTIVE_PLAYER).addEnergy(0);
    const pharan = new CardInGame(byName('Pharan'), ACTIVE_PLAYER).addEnergy(1);
    const belt = new CardInGame(byName('Orothean Belt'), ACTIVE_PLAYER);
    const seaBarl = new CardInGame(byName('Sea Barl'), ACTIVE_PLAYER);
    ...
    gameState.update(allowEffect);
    expect(gameState.getZone(ZONE_TYPE_HAND, ACTIVE_PLAYER).length).toEqual(0, 'Player has no new cards in hand')
    expect(blu.data.energy).toEqual(0, 'Blu still has 0 energy')
  })
})
